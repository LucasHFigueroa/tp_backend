  ...
  return (
    <div className="admin-dashboard">
      ...
      <div className="admin-toolbar">
        ...
        <button className="admin-btn-new" onClick={() => { setEditingProduct(null); setFormOpen(true) }}>
          + NUEVO PRODUCTO
        </button>
      </div>
      ...
      {loading && <div className="admin-loading"><div className="admin-spinner" /> cargando productos...</div>}
      ...
      {!loading && !error && (
        <div className="admin-table-wrap">
          <table className="admin-table">
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="admin-table__empty">No hay productos que coincidan</td></tr>
              )}
              {filtered.map(product => (
                <tr key={product._id}>
                  ...
                  <td className="admin-table__price">${product.price.toLocaleString('es-AR')}</td>
                  <td className="admin-table__stock">{product.stock}</td>
                  <td>
                    <div className="admin-table__tags">
                      {product.tags && product.tags.length > 0
                        ? product.tags.map(t => (
                            <span key={t} className="admin-tag" title={t}>
                              {TAG_LABELS[t] || t}
                            </span>
                          ))
                  ...
                  <td>
                    <div className="admin-table__actions">
                      <button className="admin-action admin-action--edit"
                      ...
                      {deleteId === product._id ? (
                        <div className="admin-confirm">
                          <span>¿Eliminar?</span>
                          <button className="admin-action admin-action--confirm" onClick={() => handleDelete(product._id)}>sí</button>
                          <button className="admin-action admin-action--cancel"  onClick={() => setDeleteId(null)}>no</button>
                        </div>
                      ) : (
                        <button className="admin-action admin-action--delete" onClick={() => setDeleteId(product._id)}>
                          ✕ borrar
                        </button>
                      )}
                    </div>
